...
import { LayoutDashboard, Wallet, Activity, Target, Users, LogOut, Clock, BookOpen, Bot, Menu, X, ShieldCheck, Zap, WifiOff } from 'lucide-react';
...
export const Layout: React.FC<Props> = ({ currentSection, setSection, children }) => {
  ...
  // Age Calculation Logic (Birthdate: Sept 23, 2001)
  ...
  return (
    <div className="min-h-screen font-sans flex flex-col md:flex-row overflow-hidden relative bg-obsidian text-gray-200 selection:bg-electric-blue selection:text-white">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
          {/* Main Atmosphere - Increased Blur for Smoother Gradients */}
      ...
      <aside className={`fixed md:relative z-40 w-64 h-full transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-500 cubic-bezier(0.23, 1, 0.32, 1) flex-shrink-0 border-r border-white/5 glass-panel flex flex-col bg-black/40 backdrop-blur-md`}>
        ...
        <div className="p-4 border-t border-white/5 bg-black/20">
             <button className="w-full flex items-center justify-center gap-2 px-4 py-3 text-gray-600 hover:text-spartan-red transition-all border border-transparent hover:border-spartan-red/30 rounded-lg group hover:bg-spartan-red/5 active:scale-95">
                <LogOut size={16} />
                <span className="font-bold text-xs uppercase tracking-widest group-hover:text-glow-red">System Offline</span>
             </button>
        </div>
      </aside>
