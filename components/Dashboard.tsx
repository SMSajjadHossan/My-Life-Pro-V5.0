...
import { UserProfile, Habit, FinancialState } from '../types';
import { RULES, MORNING_PROTOCOL, DAYTIME_PROTOCOL, MASCULINE_MINDSET_CHECKLIST, FINANCIAL_DISCIPLINE_CHECKLIST, RULES_OF_POWER_13, RULES_OF_ATTRACTION_7, INITIAL_STRATEGIC_OBJECTIVES } from '../constants';
import { getStoicAdvice } from '../services/geminiService';
import { Shield, Target, Zap, Activity, CheckSquare, List, Sun, Moon, Brain, Wallet, ChevronDown, ChevronUp, AlertCircle, Plus, Trash2, Save, Edit3, X, Clock, TrendingUp, Lock, Scan, BarChart, FileSpreadsheet, Server, HardDrive, Wifi, Eye, EyeOff, LayoutTemplate } from 'lucide-react';
...
export const Dashboard: React.FC<Props> = ({ profile, habits, financialData }) => {
  const [dailyMotto, setDailyMotto] = useState(RULES[0]);
  const [zenMode, setZenMode] = useState(false);
  
  // -- PERSISTENT STATE FOR CHECKLISTS --
  const [morningChecklist, setMorningChecklist] = useState<Record<string, boolean>>(() => JSON.parse(localStorage.getItem('dash_morning') || '{}'));
  const [daytimeChecklist, setDaytimeChecklist] = useState<Record<string, boolean>>(() => JSON.parse(localStorage.getItem('dash_daytime') || '{}'));
  const [mindsetChecklist, setMindsetChecklist] = useState<Record<string, boolean>>(() => JSON.parse(localStorage.getItem('dash_mindset') || '{}'));
  const [financeChecklist, setFinanceChecklist] = useState<Record<string, boolean>>(() => JSON.parse(localStorage.getItem('dash_finance') || '{}'));

  // -- EDITABLE STRATEGIC OBJECTIVES --
  ...
  // -- DAILY FROGS (TODAY) & NIGHT PLAN (TOMORROW) --
  const [dailyFrogs, setDailyFrogs] = useState<FrogTask[]>(() => {
    const saved = localStorage.getItem('dash_frogs');
    return saved ? JSON.parse(saved) : [
        { id: '1', task: 'Electronics 2 Study', priority: 'High', completed: false, notes: 'Pass with B grade' },
        { id: '2', task: 'TOEFL Practice', priority: 'Critical', completed: false, notes: 'Target 320+' }
    ];
  });
  ...
  const radarData = useMemo(() => {
      ...
      const morningDone = Object.values(morningChecklist).filter(Boolean).length;
  ...
  }, [totalNetWorth, habits, morningChecklist]);
  ...
  const handleExportData = () => {
      ...
      const getMeta = (dateStr: string) => {
          const d = new Date(dateStr);
          if(isNaN(d.getTime())) return { date: '', year: '', month: '', day: '', weekday: '' };
          return {
      ...
      // 16-COLUMN MASTER SCHEMA
      const headers = [
          'MODULE', 'DATE', 'YEAR', 'MONTH', 'DAY', 'WEEKDAY', 
          'CATEGORY', 'SUB_CATEGORY', 'ITEM_NAME', 'VALUE', 
          'STATUS', 'SOURCE', 'ACTION_PROTOCOL', 'PROBLEM_CONTEXT', 'REAL_LIFE_EXAMPLE', 'NOTES'
      ];

      const rows: string[] = [];

      // 1. WEALTH: TRANSACTIONS
      (financialData.transactions || []).forEach(t => {
          const m = getMeta(t.date);
          rows.push([
              'WEALTH', m.date, m.year, m.month, m.day, m.weekday, 
              t.category, t.subcategory || 'General', esc(t.description), t.amount, 
              'COMPLETED', `BANK_${t.bank}`, '-', '-', '-', esc(t.paymentMethod || 'Manual')
          ].join(','));
      });
      ...
      link.setAttribute('download', `MyLife_Pro_ULTIMATE_DUMP_V4_${todayStr}.csv`);
  ...
  useEffect(() => { localStorage.setItem('dash_morning', JSON.stringify(morningChecklist)); }, [morningChecklist]);
  useEffect(() => { localStorage.setItem('dash_daytime', JSON.stringify(daytimeChecklist)); }, [daytimeChecklist]);
  useEffect(() => { localStorage.setItem('dash_mindset', JSON.stringify(mindsetChecklist)); }, [mindsetChecklist]);
  useEffect(() => { localStorage.setItem('dash_finance', JSON.stringify(financeChecklist)); }, [financeChecklist]);
  useEffect(() => { localStorage.setItem('dash_objectives', JSON.stringify(objectives)); }, [objectives]);
  useEffect(() => { localStorage.setItem('dash_frogs', JSON.stringify(dailyFrogs)); }, [dailyFrogs]);
  useEffect(() => { localStorage.setItem('dash_tomorrow', tomorrowPlan); }, [tomorrowPlan]);

  // -- HANDLERS --

  const toggleItem = (item: string, setList: React.Dispatch<React.SetStateAction<Record<string, boolean>>>) => {
    setList(prev => ({...prev, [item]: !prev[item]}));
  };

  const calculateProgress = (list: Record<string, boolean>, total: number) => {
  ...
  const saveObj = () => {
    if (!editForm) return;
    setObjectives(prev => prev.map(o => o.id === editForm.id ? editForm : o));
    setEditingObjId(null);
  };

  const deleteObj = (id: string) => {
    if(confirm('Delete this objective?')) setObjectives(prev => prev.filter(o => o.id !== id));
  };
  ...
  const toggleFrog = (id: string) => {
    setDailyFrogs(prev => prev.map(f => f.id === id ? {...f, completed: !f.completed} : f));
  };

  const updateFrog = (id: string, field: keyof FrogTask, value: any) => {
    setDailyFrogs(prev => prev.map(f => f.id === id ? {...f, [field]: value} : f));
  };

  const addFrog = () => {
    setDailyFrogs([...dailyFrogs, { id: Date.now().toString(), task: '', priority: 'Medium', completed: false, notes: '' }]);
  };

  const deleteFrog = (id: string) => {
    setDailyFrogs(prev => prev.filter(f => f.id !== id));
  };

  // Render Checklist (GLASS STYLE)
  const renderChecklist = (title: string, items: string[], state: Record<string, boolean>, setter: React.Dispatch<React.SetStateAction<Record<string, boolean>>>, icon: React.ReactNode, color: string) => (
    <div className="glass-panel rounded-lg overflow-hidden h-full glass-panel-hover transition-all">
        <div className={`p-3 border-b border-white/5 flex justify-between items-center ${color}`}>
            <h4 className="font-display font-bold uppercase text-xs flex items-center gap-2 text-white">
                {icon} {title}
            </h4>
            <span className="text-[10px] font-mono bg-black/50 px-2 py-0.5 rounded text-white border border-white/10">
                {calculateProgress(state, items.length)}%
            </span>
        </div>
        <div className="p-2 space-y-1">
            {items.map((item, idx) => (
                <div key={idx} onClick={() => toggleItem(item, setter)} className="flex items-start gap-3 p-2 rounded hover:bg-white/5 cursor-pointer group">
                    <div className={`mt-0.5 min-w-[14px] h-[14px] border rounded flex items-center justify-center transition-all ${state[item] ? 'bg-wealth-green border-wealth-green shadow-[0_0_8px_#00E676]' : 'border-gray-600 group-hover:border-gray-400'}`}>
                        {state[item] && <CheckSquare size={10} className="text-black" />}
                    </div>
                    <span className={`text-[11px] leading-tight font-medium ${state[item] ? 'text-gray-500 line-through' : 'text-gray-300'}`}>{item}</span>
                </div>
            ))}
        </div>
    </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 relative">
      ...
      {showAnalysis && (
          <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setShowAnalysis(false)}>
      ...
      {zenMode ? (
          <div className="grid grid-cols-1 gap-6 animate-in fade-in">
              ...
              <div className="bg-black/60 border border-indigo-500/30 rounded-lg p-8 relative overflow-hidden group shadow-[0_0_50px_rgba(79,70,229,0.1)]">
                  <div className="flex justify-between items-center mb-8 relative z-10">
                      ...
                      <button onClick={addFrog} className="text-xs bg-indigo-600 text-white font-bold px-4 py-2 rounded hover:bg-indigo-500 transition-all">
                          + ADD TARGET
                      </button>
                  </div>
                          ...
                          <div key={frog.id} className="bg-slate-900/80 border border-gray-700/50 p-4 rounded-lg flex flex-col gap-2 group hover:border-indigo-500 transition-all shadow-lg">
                              <div className="flex items-center gap-4">
                                  <input 
                                      ...
                                      onChange={() => toggleFrog(frog.id)} 
                                      className="h-6 w-6 accent-indigo-500 bg-black border-gray-600 rounded cursor-pointer"
                                  />
                                  <input 
                                      value={frog.task} 
                                      onChange={(e) => updateFrog(frog.id, 'task', e.target.value)}
                                      placeholder="Identify Critical Task..."
                                      className={`bg-transparent outline-none flex-1 font-bold text-lg ${frog.completed ? 'text-gray-500 line-through' : 'text-white'}`}
                                  />
                                  <button onClick={() => deleteFrog(frog.id)} className="text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
                              </div>
                          </div>
                      ))}
                      {dailyFrogs.length === 0 && <p className="text-center text-gray-500 italic py-8">No active targets. Set your sights.</p>}
                  </div>
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* 2. THE MATRIX (CHECKLISTS) - COMPACT */}
                <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {renderChecklist("Morning", MORNING_PROTOCOL, morningChecklist, setMorningChecklist, <Sun size={14}/>, "bg-amber-900/10 border-amber-900/30 text-amber-500")}
                    {renderChecklist("Daytime", DAYTIME_PROTOCOL, daytimeChecklist, setDaytimeChecklist, <Activity size={14}/>, "bg-blue-900/10 border-blue-900/30 text-blue-500")}
                    {renderChecklist("Mindset", MASCULINE_MINDSET_CHECKLIST, mindsetChecklist, setMindsetChecklist, <Brain size={14}/>, "bg-purple-900/10 border-purple-900/30 text-purple-500")}
                    {renderChecklist("Finance", FINANCIAL_DISCIPLINE_CHECKLIST, financeChecklist, setFinanceChecklist, <Wallet size={14}/>, "bg-emerald-900/10 border-emerald-900/30 text-emerald-500")}
                </div>
            ...
            {/* 5. STRATEGIC OBJECTIVES (EDITABLE TABLE) */}
            <div className="glass-panel rounded-lg p-6 border-t border-white/10">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-display font-bold text-white uppercase flex items-center gap-2">
                        <List size={18} className="text-electric-blue" /> Strategic Objectives
                    </h3>
                                ...
                                <tr key={obj.id} className="border-b border-gray-800/50 hover:bg-white/5 transition-colors group">
                                    {editingObjId === obj.id ? (
                                        ...
                                        <>
                                            ...
                                            <td className="p-3 text-center font-mono text-electric-blue">{obj.progress}%</td>
            ...
            {/* 6. DAILY FOCUS & TOMORROW'S PLAN */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* EAT THAT FROG (TODAY) */}
                <div className="bg-black/40 border border-spartan-red/30 rounded-lg p-6 relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-5"><AlertCircle size={100} className="text-spartan-red"/></div>
                    <div className="flex justify-between items-center mb-6 relative z-10">
                        <div className="flex items-center gap-2">
                            <AlertCircle className="text-spartan-red animate-pulse" />
                            <h3 className="text-xl font-display font-black text-white uppercase">Today's Frogs</h3>
                        </div>
                        <button onClick={addFrog} className="text-xs bg-spartan-red text-white font-bold px-3 py-1 rounded hover:bg-red-600 shadow-[0_0_10px_rgba(220,38,38,0.4)] transition-all">
                            + ADD TASK
                        </button>
                    </div>
                    ...
                    <div className="space-y-3 relative z-10">
                        {dailyFrogs.map((frog) => (
                            <div key={frog.id} className="bg-slate-900/80 border border-gray-800 p-3 rounded flex flex-col gap-2 group hover:border-spartan-red/50 transition-colors">
                                <div className="flex items-center gap-3">
                                    <input 
                                        type="checkbox" 
                                        checked={frog.completed} 
                                        onChange={() => toggleFrog(frog.id)} 
                                        className="h-5 w-5 accent-spartan-red bg-black border-gray-700 rounded cursor-pointer"
                                    />
                                    <input 
                                        value={frog.task} 
                                        onChange={(e) => updateFrog(frog.id, 'task', e.target.value)}
                                        placeholder="Task Name..."
                                        className={`bg-transparent outline-none flex-1 font-bold ${frog.completed ? 'text-gray-500 line-through' : 'text-white'}`}
                                    />
                                    <button onClick={() => deleteFrog(frog.id)} className="text-gray-600 hover:text-spartan-red opacity-0 group-hover:opacity-100"><Trash2 size={14}/></button>
                                </div>
                                <div className="flex gap-2 pl-8">
                                    <select 
                                        value={frog.priority}
                                        onChange={(e) => updateFrog(frog.id, 'priority', e.target.value)}
                                        className="bg-black text-[10px] text-gray-400 border border-gray-800 rounded px-1 outline-none font-mono"
                                    >
                                        <option>Medium</option><option>High</option><option>Critical</option>
                                    </select>
                                    <input 
                                        value={frog.notes} 
                                        onChange={(e) => updateFrog(frog.id, 'notes', e.target.value)}
                                        placeholder="Notes/Target..."
                                        className="bg-transparent text-[10px] text-gray-500 outline-none flex-1 font-mono"
                                    />
                                </div>
                            </div>
                        ))}
                ...
                {/* NIGHT PROTOCOL (TOMORROW) */}
                <div className="glass-panel rounded-lg p-6 border-t border-white/5">
                    <div className="flex items-center gap-2 mb-4">
                        <Moon className="text-electric-blue" />
                        <h3 className="text-xl font-display font-black text-white uppercase">Night Protocol</h3>
                    </div>
                    ...
                    <textarea 
                        ...
                        className="w-full h-64 bg-black/50 border border-gray-700 rounded p-4 text-gray-300 font-mono text-sm leading-relaxed focus:border-electric-blue outline-none resize-none"
                    />
                    <div className="flex justify-between items-center mt-2">
                        <span className="text-[10px] text-gray-600 uppercase font-bold">Auto-saves to local storage</span>
                        <Clock className="text-gray-600" size={14}/>
                    </div>
                </div>
            </div>

            {/* 7. SYSTEM DATA CENTER (MASTER EXPORT) */}
            <div className="border-t border-gray-800 pt-8 mt-8">
                <div className="bg-black border border-electric-blue/30 rounded-lg p-0 relative overflow-hidden group shadow-[0_0_30px_rgba(41,121,255,0.1)]">
                    {/* Server Aesthetic Top Bar */}
                    <div className="bg-black p-2 border-b border-gray-800 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                            <Wifi size={10} className="text-electric-blue animate-pulse"/>
                            <span className="text-[9px] text-electric-blue font-mono uppercase">SYS_ONLINE</span>
                        </div>
                    </div>
                    ...
                    <div className="p-8 relative z-10 flex flex-col md:flex-row gap-8">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="bg-electric-blue/10 p-2 rounded border border-electric-blue/30">
                                    <Server size={24} className="text-electric-blue"/>
                                </div>
                                <div>
                                    <h3 className="text-2xl font-display font-black text-white uppercase tracking-tighter">System Data Center</h3>
                                    <p className="text-[10px] text-electric-blue font-mono uppercase tracking-widest mt-0.5">V4.0 • ULTIMATE ARCHIVE • COMPLETE DUMP</p>
                                </div>
                            </div>
                        ...
                        <div className="w-full md:w-1/3 flex flex-col justify-between">
                            <div className="grid grid-cols-2 gap-2 mb-4">
                                <div className="bg-slate-900 p-3 rounded border border-gray-800 text-center">
                                    <p className="text-xs text-gray-500 uppercase font-bold mb-1">Total Records</p>
                                    <p className="text-xl text-white font-mono font-bold">
                                        {(financialData.transactions?.length || 0) + 
                                        (JSON.parse(localStorage.getItem('books') || '[]').reduce((acc:number, b:any) => acc + (b.neuralNotes?.length || 0), 0)) + 
                                        (financialData.budgetSnapshots?.length || 0) + 
                                        (financialData.mindsetLogs?.length || 0) + 
                                        (JSON.parse(localStorage.getItem('academy_failures') || '[]').length)}
                                    </p>
                                </div>
                                <div className="bg-slate-900 p-3 rounded border border-gray-800 text-center">
                                    <p className="text-xs text-gray-500 uppercase font-bold mb-1">Neural-Nodes</p>
                                    <p className="text-xl text-white font-mono font-bold">
                                        {JSON.parse(localStorage.getItem('books') || '[]').reduce((acc:number, b:any) => acc + (b.neuralNotes?.length || 0), 0)}
                                    </p>
                                </div>
                            ...
                            <button 
                                ...
                                className="w-full bg-electric-blue hover:bg-blue-500 text-white font-bold py-4 px-6 rounded uppercase text-xs tracking-widest flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(41,121,255,0.3)] transition-all border border-blue-400 group-hover:scale-[1.02]"
                            >
                                <HardDrive size={18} /> INITIALIZE DUMP (.CSV)
                            </button>
                            <p className="text-[9px] text-gray-600 text-center mt-2 uppercase">Secure Connection Established</p>
                        </div>
                    </div>
